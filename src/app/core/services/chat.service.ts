...
export class ChatService{
  ...
  askSSE(q: string, params: Array<{key: string, value: string}> = []) {
    const convId = this.conv.activeId$.value || '';
    const qs = new URLSearchParams({
      q, 
      stream: 'true', 
      conversation_id: convId,
      prompt_params: JSON.stringify(params)
    });
    ...
    es.onerror = () => { 
      this.zone.run(() => this.stream$.next({ type: 'done' })); 
      es.close(); 
    };
    return es;
  } 

  async askOnce(q: string, params: Array<{key: string, value: string}> = []) {
    const body: AskPayload = {
      query: q,
      conversation_id: this.conv.activeId$.value,
      stream: false,
